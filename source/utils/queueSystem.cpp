	...
	while(QueueRuns) {
		...
		if (!QueueSystem::Wait) {
			...
			if (queueEntries[0]->status == QueueStatus::Done) { // ONLY update, if successful.
				if (StoreUtils::meta) {
					StoreUtils::meta->SetUpdated(queueEntries[0]->eshopName, queueEntries[0]->entryName, queueEntries[0]->lastUpdated);
					StoreUtils::meta->SetInstalled(queueEntries[0]->eshopName, queueEntries[0]->entryName, queueEntries[0]->name);
